...
class AtendimentoService implements AtendimentoServiceInterface
{
    ...
    public function chamarSenha(AtendimentoInterface $atendimento, UsuarioInterface $usuario): void
    {
        ...
        $this->logger->info('Chamando notificaPainel para atendimento', [
            'atendimento_id' => $atendimento->getId(),
            'unidade_id' => $atendimento->getUnidade()->getId(),
        ]);
        $this->mercureService->notificaPainel($atendimento);
        $this->logger->info('notificaPainel chamado com sucesso');
    }
    ...
    public function chamarProximo(
        ...
        do {
            ...
            if ($success) {
                // Chama a senha para notificar o painel
                $this->chamarSenha($proximo, $usuario);
            } else {
                usleep(100);
            }
            --$attempts;
    ...
    public function getAtendimentoAndamento(
        ...
        try {
            ...
            $this
                ...
                ->set('e.usuario', ':null')
                ->where('e.usuario = :usuario')
                ->andWhere('e.status IN (:statusIn)')
                ->setParameter('novoStatus', self::SENHA_EMITIDA)
                ->setParameter('null', null)
                ->setParameter('usuario', $usuario)
                ->setParameter('statusIn', $status)
                ->getQuery()
                ->execute();

...
class MercureService
{
    ...
    {
        try {
            $update = new Update($topics, json_encode($params));
            $this->logger->info('Mercure: Publicando atualização', [
                'topics' => $topics,
                'data' => $params,
            ]);
            $this->hub->publish($update);
            $this->logger->info('Mercure: Atualização publicada com sucesso', [
                'topics' => $topics,
            ]);
        } catch (RuntimeException $ex) {
            $this->logger->error('Mercure: Erro ao publicar atualização', [
                'topics' => $topics,
                'error' => $ex->getMessage(),
                'trace' => $ex->getTraceAsString(),
            ]);
        } catch (\Throwable $ex) {
            $this->logger->error('Mercure: Erro inesperado ao publicar atualização', [
                'topics' => $topics,
                'error' => $ex->getMessage(),
                'class' => get_class($ex),
                'trace' => $ex->getTraceAsString(),
